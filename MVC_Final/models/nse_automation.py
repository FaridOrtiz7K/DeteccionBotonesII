...
class NSEAutomation:
    def __init__(self, csv_file, linea_especifica=None):
        """
        Inicializa la automatización NSE.
        ...
        # Coordenadas relativas a la imagen de referencia
        self.coords_select = {
    ...
    def should_skip_process(self, row):
        # Verifica si la columna 6 (índice 5) tiene algún valor no vacío para saltar
    ...
    def execute_nse_script(self):
        """
        Ejecuta el script de automatización NSE para la línea especificada.
        ...
        try:
            ...
            # Verificar si debemos saltar este proceso
            if self.should_skip_process(row):
                logger.info(f"⏭️  Saltando línea {self.linea_especifica} - Columna 6 tiene valor: {row.iloc[5]}")
                return True
            
            # Verificar tipo V en columna 5 (índice 4)
            if str(row.iloc[4]).strip().upper() != "V":
                logger.info(f"⚠️  Saltando línea {self.linea_especifica} - No es tipo V: {row.iloc[4]}")
                return True
    ...
    def handle_type_v(self, row, base_location):
        """
        Maneja la entrada de datos para tipo V.
        """
        ...
        for col_index in range(7, 18):
            ...
            # Verificar si la celda tiene valor positivo
