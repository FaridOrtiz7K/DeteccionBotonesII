...
class AHKController:
    ...
    def execute_nse(self):
        """Ejecuta el script NSE completo"""
        return self.send_command("EXECUTE_NSE")
...
def print_header():
    ...
    print("=" * 60)
    print("          CONTROLADOR NSE - AUTOMATIZACIÓN")
    print("=" * 60)
    print()
...
def main():
    ...
    if not success:
        ...
        print()
        print("Asegúrate de que el script NSE_Automation.ahk esté en ejecución")
    ...
    try:
        ...
        # Ejecutar script NSE
        success, response = ahk.execute_nse()
        if not success:
            print(f"❌ Error al iniciar ejecución: {response}")
            input("Presiona Enter para salir...")
            return
        
        print("✅ Ejecución NSE iniciada")
        print()
                    ...
                    if "RUNNING" in status:
                        ...
                        if success and "PROGRESS" in progress:
                            ...
                            if len(parts) >= 2:
                                ...
                                if current_line != last_progress:
                                    print(f"📊 Procesando línea: {current_line}")
