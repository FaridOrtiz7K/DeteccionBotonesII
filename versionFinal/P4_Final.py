...
class CSVSelectorGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Seleccionar Archivo CSV")
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(
            self.root, 
            text="Automatización GEAutomation", 
            font=("Arial", 16, "bold"),
            bg="#f0f0f0",
            fg="#333333"
        )
        ...
        subtitle_label = tk.Label(
            self.root,
            text="Seleccione el archivo CSV con los datos a procesar",
            font=("Arial", 10),
            bg="#f0f0f0",
            fg="#666666"
        )
    ...
    def select_csv_file(self):
        """Abre el diálogo para seleccionar archivo CSV"""
        ...
        if file_path:
            ...
            try:
                ...
                row_count = len(df)
                info_text = f"✓ Archivo válido\n✓ {row_count} registros encontrados"
                if hasattr(df, 'columns'):
                    info_text += f"\n✓ {len(df.columns)} columnas detectadas"
                
                # Crear ventana de información
                info_window = tk.Toplevel(self.root)
                info_window.title("Información del CSV")
                info_window.geometry("400x200")
                info_window.configure(bg="#f0f0f0")
                
                # Centrar ventana de información
                ...
                tk.Label(
                    info_window,
                    text="📊 Información del Archivo CSV",
                    font=("Arial", 12, "bold"),
                    bg="#f0f0f0"
                ).pack(pady=10)
    ...
    def start_automation(self):
        ...
        confirm = messagebox.askyesno(
            "Confirmar inicio",
            f"¿Está seguro de iniciar la automatización con el archivo:\n{os.path.basename(self.csv_file)}?\n\n"
            "Asegúrese de tener:\n"
            "1. Google Earth Pro abierto\n"
            "2. Las ventanas en la posición correcta\n"
            "3. Preparado para iniciar"
        )
        
        if confirm:
            self.root.destroy()
            run_automation(self.csv_file)

def run_automation(csv_file):
    """Función principal que ejecuta la automatización"""
    # Inicializar automatización
    ge_auto = GEAutomation(csv_file)
